...
export class GeneroListaComponent implements OnInit {
  ...
  cargarFormatos() {
    ...
    this.generoService.obtenerGeneros(miFiltro, this.paginaActual(), this.tamanioPagina()).subscribe({
      next: (respuesta) => {
        this.generos.set(respuesta.registros);
      ...
      accept: () => {
        this.generoService.eliminarGenero(id).subscribe({
          next: () => {
            ...
            if (this.generos().length === 1 && this.paginaActual() > 1) {
